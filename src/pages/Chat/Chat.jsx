    ...
    return (
        <div className="flex flex-col h-screen bg-white ">
            ...
            <div className="flex flex-1">
                {/* Sidebar - User List */}
                <div
                    className={`absolute md:relative top-0 left-0 w-80 md:w-96 h-full bg-white md:flex flex-col border-r border-gray-200 transition-all duration-300 z-50 ${showSidebar ? "translate-x-0" : "-translate-x-full md:translate-x-0"
                        }`}
                >
                    <div className="p-4">
                        <button
                    ...
                    <div className="overflow-y-auto flex-1">
                        {users.map((user) => (
                            <div
                                key={user.id}
                                className={`flex items-center gap-3 p-4 cursor-pointer border-b border-gray-100 hover:bg-gray-50 transition-colors ${selectedUser.id === user.id ? 'bg-yellow-50 border-r-4 border-yellow-400' : ''
                                    }`}
                                onClick={() => {
                ...
                <div className="flex-1 flex flex-col">
                    ...
                    <div className="flex-1 overflow-auto bg-gray-50 p-4 space-y-4">
                        {messages.map((msg, i) => (
                            ...
                            >
                                <div className={`max-w-xs lg:max-w-md xl:max-w-lg rounded-2xl p-4 ${msg.sender === "me"
                                    ...
                                    <div className="flex items-center justify-between mt-2">
                                        ...
                                        {msg.sender === "me" && (
                                            <IoCheckmarkDone className={`w-4 h-4 ${msg.read ? 'text-blue-500' : 'text-gray-400'
                                                }`} />
                                        )}
                                    </div>
                                </div>
